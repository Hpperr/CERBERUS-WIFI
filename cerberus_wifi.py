#!/usr/bin/env python3
"""
CERBERUS-WIFI v1.0 - Advanced WiFi Deauth & Device Control Framework
Professional WiFi Security Testing & Device Management

Copyright (c) 2024 F1REW0LF
License: MIT - For authorized security testing only

Usage: sudo python3 cerberus_wifi.py -i wlan0
"""
...
class DeviceDatabase:
    ...
    # OUI lookup table (MAC prefix -> Manufacturer)
    OUI_DB = {
        # Major manufacturers
        '00:11:22': 'Cisco',
        '00:1a:3f': 'Sony',
    ...
    def lookup_manufacturer(cls, mac: str) -> str:
        """Look up manufacturer by MAC prefix"""
        if not mac:
            return "Unknown"
        mac = mac.upper()
    ...
    def detect_device_type(cls, hostname: str, user_agent: str, mac: str) -> str:
        ...
        if manufacturer == "Apple":
            ...
            return "Android"
        elif manufacturer in ["Cisco", "D-Link", "Netgear"]:
...
class WiFiController:
    ...
    def channel_hopper(self):
        """Hop through channels for full coverage"""
        channels = list(range(1, 12))  # 2.4GHz channels
    ...
    def packet_handler(self, pkt):
        ...
        # Process beacon frames (AP detection)
        if pkt.haslayer(Dot11Beacon):
            self._handle_beacon(pkt)
        
        # Process probe requests (device detection)
        if pkt.haslayer(Dot11ProbeReq):
            self._handle_probe_request(pkt)
        
        # Process data frames (client info)
        if pkt.haslayer(Dot11):
            self._handle_data_frame(pkt)
        
        # Process DHCP (hostname detection)
        if pkt.haslayer(DHCP):
            self._handle_dhcp(pkt)
        
        # Process DNS (device info)
        if pkt.haslayer(DNS):
    ...
    def _handle_beacon(self, pkt):
        ...
        try:
            ...
            for elt in pkt.iterpayloads():
                if hasattr(elt, 'ID') and elt.ID == 3:  # DSset
                    channel = elt.info[0]
                    break
    ...
    def _handle_probe_request(self, pkt):
        """Handle probe requests (device discovery)"""
        try:
            mac = pkt.addr2
        ...
        try:
            ...
            if src in self.devices:
                with self.lock:
                    ...
                    if bssid:
                        self.devices[src]['bssid'] = bssid
        except:
            pass
    
    def _handle_dhcp(self, pkt):
        """Handle DHCP packets for hostnames"""
        try:
            if pkt.haslayer(DHCP):
                # Get hostname
    ...
    def _add_device(self, mac: str, pkt):
        ...
        # Try to get hostname from packet
        hostname = "Unknown"
        if pkt.haslayer(Dot11ProbeReq):
            # Check for SSID in probe request
        ...
        if mac in self.banned_devices:
            cprint(f"      [!] This device is BANNED - Deauth active", Colors.RED)
    ...
    def deauth_device(self, mac: str, bssid: str = "FF:FF:FF:FF:FF:FF", count: int = 100):
        """Send deauth packets to a device"""
        if not bssid or bssid == "FF:FF:FF:FF:FF:FF":
            # Try to find BSSID from device info
            with self.lock:
    ...
    def list_devices(self):
        ...
        with self.lock:
            for idx, (mac, info) in enumerate(self.devices.items(), 1):
                ...
                print(f"{idx:<4} {mac:<20} {info.get('device_type', 'Unknown'):<15} {info.get('vendor', 'Unknown'):<12} {color}{status:<10}{Colors.WHITE} {info.get('hostname', 'Unknown'):<20}")
    ...
    def run_menu(self):
        ...
        while self.running:
            ...
            print("7. Refresh Scan (Continue)")
            print("8. Exit")
            
            choice = input(f"\n{Colors.CYAN}[>] Select (1-8): {Colors.WHITE}").strip()
            ...
            if choice == '1':
                ...
                cprint("[*] Continuing scan...", Colors.BLUE)
            elif choice == '8':
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CERBERUS-WIFI - WiFi Deauth & Device Control Framework",
...
Examples:
  ...
  sudo python3 cerberus_wifi.py -i wlan0 --list
    ...
    parser.add_argument("--list", action="store_true", help="List all devices and exit")
    ...
    if args.list:
        # Start scanning briefly to collect devices
